...
from metavision.annotators.base import ImageType
from metavision.detection.utils import pad_boxes, spread_out_boxes
from metavision.draw.color import Color
from metavision.draw.utils import draw_rounded_rectangle
from metavision.geometry.core import Rect
from metavision.keypoint.core import KeyPoints
from metavision.keypoint.skeletons import SKELETONS_BY_VERTEX_COUNT
from metavision.utils.conversion import ensure_cv2_image_for_annotation
...
class VertexAnnotator(BaseKeyPointAnnotator):
    ...
    def annotate(self, scene: ImageType, key_points: KeyPoints) -> ImageType:
        ...
        metavision-annotator-examples/vertex-annotator-example.png)
        """
...
class EdgeAnnotator(BaseKeyPointAnnotator):
    ...
    def annotate(self, scene: ImageType, key_points: KeyPoints) -> ImageType:
        ...
        metavision-annotator-examples/edge-annotator-example.png)
        """
        assert isinstance(scene, np.ndarray)
        if len(key_points) == 0:
            return scene
...
class VertexLabelAnnotator:
    ...
    def annotate(
        ...
        metavision-annotator-examples/vertex-label-annotator-example.png)
        ...
        metavision-annotator-examples/vertex-label-annotator-custom-example.png)
        """
        assert isinstance(scene, np.ndarray)
