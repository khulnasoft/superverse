...
class MetavisionWarnings(Warning):
    """Metavision warning category.
    Set the deprecation warnings visibility for Metavision library.
...
if os.getenv("SUPERVISON_DEPRECATION_WARNING") == "0":
    warnings.simplefilter("ignore", MetavisionWarnings)
else:
    warnings.simplefilter("always", MetavisionWarnings)
...
def warn_deprecated(message: str):
    ...
    """
    warnings.warn(message, category=MetavisionWarnings, stacklevel=2)
