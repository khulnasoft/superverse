...
import cv2
import numpy as np
from inference.core.models.khulnasoft import KhulnasoftInferenceModel
...
def detect(
    ...
    Args:
        ...
        model (KhulnasoftInferenceModel): The Inference model used for processing the
            frame.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Khulnasoft model ID",
        type=str,
    )
    parser.add_argument(
        "--khulnasoft_api_key",
        default=None,
        help="Khulnasoft API KEY",
        type=str,
    )
    parser.add_argument(
        "--source_video_path",
    ...
    api_key = args.khulnasoft_api_key
    api_key = os.environ.get("KHULNASOFT_API_KEY", api_key)
    if api_key is None:
        raise ValueError(
            "Khulnasoft API key is missing. Please provide it as an argument or set the "
            "KHULNASOFT_API_KEY environment variable."
        )
    args.khulnasoft_api_key = api_key
    ...
    model = get_khulnasoft_model(model_id=args.model_id, api_key=args.khulnasoft_api_key)
